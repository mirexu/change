...
async function pickMergeFiles(): Promise<void> {
  const picked = await window.api.pickFiles(VIDEO_PICK_FILTER)
  if (picked.length) {
...
function onMergeDrop(e: DragEvent): void {
...
async function doMerge(): Promise<void> {
  ...
  await window.api.addTask(mergeFiles.value[0], opts)
  mergeFiles.value = []
...
function onFrameDrop(e: DragEvent): void {
  frameDragging.value = false
  const paths = Array.from(e.dataTransfer?.files ?? [])
    .map((f) => window.api.getPathForFile(f))
    .filter((p) => p && p.length > 0)
  if (paths.length) frameFile.value = paths[0]
}
...
async function doFrame(): Promise<void> {
  ...
  await window.api.addTask(frameFile.value, opts)
  frameFile.value = ''
...
function onFileMenu(cmd: string): void {
  ...
  else if (cmd === 'quit') window.api.quitApp()
}
...
onMounted(async () => {
  ...
  const encoders = await window.api.listEncoders()
